...
import { addMentor } from '../../../store/mentors/actions';
...
export const MentorsPanel = () => {
  ...
  const onFormChange = useCallback(
    (event) => {
      ...
      const newValue =
        name === 'haveCert' || name === 'instagramAd' ? checked : value;
  ...
  const { fullName, salary, salaryType } = form;
    ...
    <>
      ...
      >
        ...
        >
          ...
          <Form.Item label="тип оплати">
            ...
            >
              {Object.keys(salaryTypes).map((t) => (
                <Radio key={t} value={salaryTypes[t]}>
                  {t}
                </Radio>
              ))}
            </Radio.Group>
          ...
          <Form.Item
            label={salaryType === salaryTypes.FIX ? 'грн / урок' : '% від ск'}
          >
            <Input
              type="number"
              name="salary"
