...
import { Form, Input, Button, Select, Modal, Card, DatePicker } from 'antd';

import { addGroup } from '../../../store/groups/actions';
import { initialGroup, weekDayTypes } from '../../../constants';
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap' }}>
        {groups &&
          groups.map(
            ({ mentorId, memberIds, name: n, schedule: sc, ...rest }) => (
              <div style={{ padding: 32, width: '50%' }}>
                <Card title={n}>
                  <p>{`Групу веде ${getMentor(mentorId).fullName}`}</p>
                  <p>
                    {`В группі навчаються ${memberIds.map(
                      (memberId) => `${getStudent(memberId).fullName} `
                    )}`}
                  </p>
                  <p>Розклад</p>
                  {sc.map((key) => (
                    <p>{`${weekDayTypes[key.toUpperCase()]} - ${rest[key]}`}</p>
                  ))}
                </Card>
              </div>
            )
